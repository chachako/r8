...
/**
 * Records resource usages, detects usages of WebViews and {@code Resources#getIdentifier}, gathers
 * string constants from compiled .dex files.
 *
 * @param root directory starting from which all .dex files are analyzed.
 */
public class DexUsageRecorder(private val root: Path) : ResourceUsageRecorder {
...
public class DexFileAnalysisCallback(
  ...
  // resource as used.
  private val visitingMethod = MethodVisitingStatus()
  ...
  private fun isResourceClass(internalName: String): Boolean {
    val realClassName =
  ...
  override fun referencedInt(value: Int) {
    ...
    if (ResourceUsageModel.markReachable(resource)) {
      model.debugReporter.debug { "Marking $resource reachable: referenced from $path" }
    }
  }
  ...
  override fun referencedStaticField(internalName: String, fieldName: String) {
    ...
    val realMethod =
      model.obfuscatedClasses.resolveOriginalMethod(
    ...
    if (isValidResourceType(realMethod.className)) {
      ...
      ResourceType.fromClassName(typePart)?.let { type ->
        model.resourceStore.getResources(type, realMethod.methodName).forEach {
  ...
  override fun referencedString(value: String) {
    ...
    if (
      value.all { Character.isJavaIdentifierPart(it) || isSpecialCharacter(it) } &&
  ...
  override fun referencedMethod(
    ...
    if (
      internalName == "android/content/res/Resources" &&
  ...
  private fun shouldIgnoreField(): Boolean {
    val visitingFromStaticInitRClass =
      (isRClass && visitingMethod.isVisiting && (visitingMethod.methodName == "<clinit>"))
